...
async function fuzzyFindSite(user, requestedSite) {
  ...
  const query = requestedSite.toLowerCase();
  let best = null;
  let bestDistance = Infinity;
  for (const candidate of candidates) {
    const distance = levenshtein(query, (candidate.siteName || '').toLowerCase());
    if (distance < bestDistance) {
      bestDistance = distance;
      best = candidate;
    }
  }
  // Allow roughly up to ~40% of the query's length to differ (typos, not a different word).
...
async function getTicketSummary(user, { site, status, priority } = {}) {
  ...
  const [byStatus, slaBreached, sample] = await Promise.all([
    ...
    Ticket.find(query).sort({ createdAt: -1 }).limit(5)
      .select('ticketId title status priority siteId createdAt').lean()
  ]);
  ...
  return {
    ...
    recentTickets: sample.map((t) => ({
      ticketId: t.ticketId, title: t.title, status: t.status, priority: t.priority
    }))
  };
}
...
export const TOOL_DECLARATIONS = [
  {
    ...
    description: 'Get ticket counts, SLA breach counts and recent tickets, optionally filtered by site/status/priority.',
    parameters: {
      type: 'object',
      properties: {
