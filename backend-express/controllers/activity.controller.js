...
export const getActivities = async (req, res, next) => {
  try {
    ...
    const activitiesWithAttachments = activities.map(activity => ({
      ...
      userName: activity.isSystem ? 'TicketOps VLAccess' : (activity.userId?.fullName || 'Unknown'),
      userRole: activity.isSystem ? 'System' : (activity.userId?.role || ''),
      userAvatar: activity.userId?.profilePicture || null,
      isSystem: activity.isSystem || false,
